...
Epochs <- seq.Date(as.Date(min(platcov_dat$Rand_date)), as.Date(max(platcov_dat$Rand_date)), "3 month")

platcov_dat$Epoch = 0
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[2])] = 1 #"2021-12-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[3])] = 2 #"2022-03-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[4])] = 3 #"2022-06-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[5])] = 4 #"2022-09-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[6])] = 5 #"2022-12-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[7])] = 6 #"2023-03-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[8])] = 7 #"2023-06-29"
platcov_dat$Epoch[platcov_dat$Rand_date > as.Date(Epochs[9])] = 8 #"2023-09-29"
...
stan_inputs[[i]] = 
  make_stan_inputs(input_data_fit = platcov_dat_analysis_list[[i]],
                   ...
                   trt_frmla = formula('~ Trt * Epoch - Epoch'), #'~ Trt'
                   Dmax = Dmax[i])
...
Gr_mod4 <-pred_mod4 %>%
  ...
  scale_alpha_manual(values = rep(0.6, length(vars)), guide = "none") +
  ggtitle("Temporal dynamics of baseline viral densities by time since symptom onset") +
  scale_y_continuous(labels=label_math(), breaks = seq(0,9,2)) 
...
G_hl_trend <- ggplot() +
  ...
  geom_rug(data = data_for_plot_slope[data_for_plot_slope$Trt %in% c("No study drug", "Ivermectin", "Remdesivir") &
           ...
           aes(x = Rand_date, y = slope_to_hl(slope_med), col = VariantClass_new),
           sides = "tl", outside = F,
           length = unit(0.025, "npc"), linewidth = 0.75
   )
G_hl_trend
```
...
#######################################
#No study drug
slope_before <- median(slope_trt_for_summarize$slope[ind_tab$min_index[ind_tab$Trt == "No study drug"],])

slope_after <- median(slope_trt_for_summarize$slope[ind_tab$max_index[ind_tab$Trt == "No study drug"],])

relative_NSD <- formatter(quantile(slope_to_hl(slope_trt_for_summarize$slope[ind_tab$max_index[ind_tab$Trt == "No study drug"],])/
                   slope_to_hl(slope_trt_for_summarize$slope[ind_tab$min_index[ind_tab$Trt == "No study drug"],]),
                   c(0.025, 0.5, 0.975)))

#Nirmatrelvir
slope_before_NMT <- median(slope_trt_for_summarize$slope[ind_tab$min_index[ind_tab$Trt == "Nirmatrelvir + Ritonavir"],])
slope_after_NMT <- median(slope_trt_for_summarize$slope[ind_tab$max[ind_tab$Trt == "Nirmatrelvir + Ritonavir"],])

relative_NMT <- formatter(quantile(slope_to_hl(slope_trt_for_summarize$slope[ind_tab$max_index[ind_tab$Trt == "Nirmatrelvir + Ritonavir"],])/
                   slope_to_hl(slope_trt_for_summarize$slope[ind_tab$min_index[ind_tab$Trt == "Nirmatrelvir + Ritonavir"],]),
                   c(0.025, 0.5, 0.975)))
#######################################
writeLines(sprintf('In no study drug arm:\n September 2021: Viral clearance rate = %s log10 units/day; Half-life = %s hours\n October 2023:  Viral clearance rate = %s log10 units/day; Half-life = %s hours (Equilvalent to a decrease of %s%% [%s; %s%%] in half-life)\n\n\nIn nirmatrelvir arm: \n Jun 2022: Viral clearance rate = %s log10 units/day; Half-life = %s hours\n October 2023:  Viral clearance rate = %s log10 units/day; Half-life = %s hours (Equilvalent to a decrease of %s%% [%s; %s%%] in half-life)',
               round(slope_before,2),
               round(slope_to_hl(slope_before),1),
               round(slope_after,2),
               round(slope_to_hl(slope_after),1),
               round(relative_NSD[2],1),
               round(relative_NSD[1],1),
               round(relative_NSD[3],1),
               round(slope_before_NMT,2),
               round(slope_to_hl(slope_before_NMT),1),
               round(slope_after_NMT,2),
               round(slope_to_hl(slope_after_NMT),1),
               round(relative_NMT[2],1),
               round(relative_NMT[1],1),
               round(relative_NMT[3],1)
               ))
...
start_slope_NSD <- slope_trt_for_summarize$slope[ind_tab$min_index[ind_tab$Trt == "No study drug"],]
recent_slope_NSD <- slope_trt_for_summarize$slope[ind_tab$max_index[ind_tab$Trt == "No study drug"],]


quantile(24*log10(2)/(-(recent_slope_NSD)), c(0.025, 0.5, 0.975))
```
