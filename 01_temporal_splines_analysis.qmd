...
# platcov_dat$VariantClass_new <- factor(platcov_dat$VariantClass_new, levels = c("Delta", "BA.1",  "BA.2" ,"BA.2.75" , "BA.4"  , 
...
```{r}
Baseline_data$vac_dose_gr <- "More than 2 doses"
...
Baseline_data$mrna_gr <- "More than 2 mRNA vaccine doses"
...
vaccine_lab <- Baseline_data %>%
  group_by(vac_dose_gr, mrna_gr) %>%
  summarise("lab" = paste0("n = ", n()))

G_vaccine <- ggplot(Baseline_data) +
  geom_jitter(aes(x = as.Date(Rand_date), y = mrna_gr, col = Time_since_last_dose#, shape = as.factor(time_vac_type)
                  ),
              size = 3, alpha = 0.85, height = 0.05)  +
  ...
  geom_text(data = vaccine_lab, aes(x = as.Date("2021-05-01"), y = mrna_gr,
                                    label = lab), hjust = 0)

```

```{r}
png(("Plots/Vaccine_over_time.png"), width = 10, height = 6, units = "in", res = 400)
G_vaccine
dev.off()
