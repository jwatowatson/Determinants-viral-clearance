...
plot_dat$Epoch_2 <- "Before Jan 2023" #"Jul 2022 to Dec 2022"
...
plot_dat$Epoch_2 <- factor(plot_dat$Epoch_2, levels = c("Before Jan 2023", "After Jan 2023"))
                             #c("Before Jul 2022", "Jul 2022 to Dec 2022", "After Jan 2023"))
...
plot_dat_daily <- plot_dat_daily %>% filter(Trt %in% c("No study drug", "Nirmatrelvir + Ritonavir"))

levels(plot_dat_daily$Trt)[levels(plot_dat_daily$Trt) == "Nirmatrelvir + Ritonavir"] <- "Nirmatrelvir"
...
```{r}
png("Plots/Figx_dynamics_before_after.png", width = 10, height = 5, units = "in", res = 350)
Gr_dynamics
dev.off()
```
...
```{r}
data_for_plot_slope$half_life_med <- 24*log10(2)/(-(data_for_plot_slope$slope_med))
data_for_plot_slope$half_life_trt_med <- 24*log10(2)/(-(data_for_plot_slope$slope_trt_med))

data_for_plot_slope %>%
  filter(Trt == "No study drug") %>%
  summarise(min = min(half_life_trt_med),
            max = max(half_life_trt_med))

data_for_plot_slope %>%
  filter(Trt == "Nirmatrelvir + Ritonavir") %>%
  summarise(min = min(half_life_trt_med),
            max = max(half_life_trt_med))

plot(data_for_plot_slope$Rand_date, data_for_plot_slope$half_life_trt_med)
