...
```{r}
data_for_plot_slope$half_life_trt_med <- 24*log10(2)/(-(data_for_plot_slope$slope_trt_med))

#No study drug
data_NSD <- data_for_plot_slope[data_for_plot_slope$Trt == "No study drug",]

slope_before <- data_NSD[,c("slope_trt_med", "half_life_trt_med")][data_NSD$Rand_date == min(data_NSD$Rand_date),][1,]
slope_after <- data_NSD[,c("slope_trt_med", "half_life_trt_med")][data_NSD$Rand_date == max(data_NSD$Rand_date),][1,]

#Nirmatrelvir
data_NMT <- data_for_plot_slope[data_for_plot_slope$Trt == "Nirmatrelvir + Ritonavir",]

slope_before_NMT <- data_NMT[,c("slope_trt_med", "half_life_trt_med")][data_NMT$Rand_date == min(data_NMT$Rand_date),][1,]
slope_after_NMT <- data_NMT[,c("slope_trt_med", "half_life_trt_med")][data_NMT$Rand_date == max(data_NMT$Rand_date),][1,]


writeLines(sprintf('In no study drug arm:\n September 2021: Viral clearance rate = %s log10 units/day; Half-life = %s hours\n October 2023:  Viral clearance rate = %s log10 units/day; Half-life = %s hours\n\nIn nirmatrelvir arm: \n Jun 2022: Viral clearance rate = %s log10 units/day; Half-life = %s hours\n October 2023:  Viral clearance rate = %s log10 units/day; Half-life = %s hours',
               round(slope_before[1],2),
               round(slope_before[2],1),
               round(slope_after[1],2),
               round(slope_after[2],1),
               round(slope_before_NMT[1],2),
               round(slope_before_NMT[2],1),
               round(slope_after_NMT[1],2),
               round(slope_after_NMT[2],1)
               ))
