...
vl_cal_m4 <- function(A0, B0, lambda1, lambda2, t){
  vl_t <-   ((lambda1/(lambda2-lambda1)) * A0 * (exp(-lambda1 * t) - exp(-lambda2 * t))) + (B0 * (exp(-lambda2 * t)))
...
A0 <- c(4,6,8)
B0 <- c(4, 6, 8)
lambda2 <- c(0.1, 0.2, 0.4)
lambda1 <- c(0.15, 0.3, 0.6)
...
Mod4_combined <- ggarrange(G1, G2, G3, G4, ncol = 2, nrow = 2, align = "hv")
Mod4_combined
