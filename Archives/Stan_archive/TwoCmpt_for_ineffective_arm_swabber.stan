...
parameters{
  ...
  // Measurement error
  real<lower=0> sigma0_logvl;
  real theta[n_swabber];
...
transformed parameters{
  ...
  // calculate predicted log viral load under the model parameters
    ...
    sigma_logvl[i] = sigma0_logvl*exp(theta[i]);
}
...
model{
  ...
  for(i in 1:N_obs)
  log_10_vl[i] ~ student_t(t_dof, pred_log10_vl[i], sigma0_logvl*exp(theta[swabber_id[i]]));
  
  // Censored observations
  for(i in (N_obs+1):Ntot){
    target += student_t_lcdf(log10_cens_vl[i] | t_dof, pred_log10_vl[i], sigma0_logvl*exp(theta[swabber_id[i]]));
  }
  ...
  sigma0_logvl ~ normal(sigma_logvl_mean,sigma_logvl_sd) T[0,];
  theta ~ normal(0,1);
  // Population parameters
