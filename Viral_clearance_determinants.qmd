...
platcov_dat = platcov_dat %>% 
  filter(!Trt %in% c('Ensitrelvir','Fluoxetine',
                     'Evusheld',
...
platcov_dat$ID = gsub(pattern = 'LA8',replacement = 'LA08',x = platcov_dat$ID,fixed = T)
platcov_dat$ID = gsub(pattern = 'PK1',replacement = 'PK01',x = platcov_dat$ID,fixed = T)

if(!all(platcov_dat$ID %in% itt_population_all$ID)) {
  print(unique(platcov_dat$ID[!platcov_dat$ID %in% itt_population_all$ID]))
  stop('IDs do not match ITT population')
}
...
mod_symptoms = mgcv::gam(Symptom_onset ~ s(Study_time), data = Baseline_data %>% filter(Site=='th001'))

mod_base_VL = mgcv::gam(Baseline.viral.load ~ s(Symptom_onset,k=3)+s(Study_time)+Fever_Baseline,
                        data = Baseline_data %>% filter(Site=='th001'))
summary(mod_base_VL)
```
