...
/**
  ...
  - Analysis on the copies per ml scale (batch effect adjustment done without uncertainty propagation)
  - Adjustment for RNaseP
  - Covariate adjustment (optional)
  - Panelized B-spline on intercept and slope
**/

// Function to build B-spline from https://github.com/milkha/Splines_in_Stan
functions {
  vector build_b_spline(real[] t, real[] ext_knots, int ind, int order);
  vector build_b_spline(real[] t, real[] ext_knots, int ind, int order) {
    // INPUTS:
    // t: the points at which the B-spline is calculated
    // ext_knots: the set of extended knots
    // ind: the index of the B-spline
...
data {
  ...
  real<lower=0, upper=Time_max> obs_day[Ntot];  // Time since randomization for sample
  ...
  real study_time[n_id];                       // calendar time of the randomisation (normalised to center at 0)
  ...
  real sigma_logvl_mean;   // prior mean of sd in the error model
  real sigma_logvl_sd;     // prior sd of sd in the error model
  ...
  // Temporal spline for intercepts
...
parameters {
  ...
  real<lower=0> sigma_logvl;           // Measurement error
...
model {
  ...
  L_Omega ~ lkj_corr_cholesky(2); // Covariance matrix - random effects for individuals
