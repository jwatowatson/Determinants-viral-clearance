...
get_itt_population = function(){
  ...
  # TH58 and TH57 used enveloppes so this is not recorded in the data files
...
f_nonlinear = function(xs, alpha_0, beta_0, tmax, trt_effect=0){
  a =  beta_0[1] * exp(-trt_effect)
  b = beta_0[2] * exp(trt_effect);
  dx = cbind(log(b)-a*(xs-tmax), log(a)+b*(xs-tmax))
  ys = alpha_0+log(a+b)-apply(dx, 1, matrixStats::logSumExp)
  ys
}


make_cov_matrices_stan = function(stan_input_job, analysis_data_stan, i_val){
  
  
  x_intercept = stan_input_job$cov_matrices$X_int[[model_settings$cov_matrices[i_val]]]
  ...
  x_slope = stan_input_job$cov_matrices$X_slope[[model_settings$cov_matrices[i_val]]]
  ...
  # x_tmax = stan_input_job$cov_matrices$X_tmax[[model_settings$cov_matrices[i_val]]]
  # if(ncol(x_tmax)==0) x_tmax = array(0, dim=c(nrow(x_tmax),1))
  # analysis_data_stan$x_tmax = x_tmax
  # analysis_data_stan$K_cov_tmax=ncol(x_tmax)
  
  return(analysis_data_stan)
}


set_trt_matrices_counterfactual = function(analysis_data_stan, tt, counterfactual_settings){
  for(tt_all in counterfactual_settings){
    if(tt==tt_all & tt_all %in% colnames(analysis_data_stan$x_intercept)){
      analysis_data_stan$x_intercept[,tt_all]=1
    } 
    if(tt!=tt_all & tt_all %in% colnames(analysis_data_stan$x_intercept)) {
      analysis_data_stan$x_intercept[,tt_all]=0
    }
    
    if(tt==tt_all & tt_all %in% colnames(analysis_data_stan$x_slope)){
      analysis_data_stan$x_slope[,tt_all]=1
    } 
    if(tt!=tt_all & tt_all %in% colnames(analysis_data_stan$x_slope)) {
      analysis_data_stan$x_slope[,tt_all]=0
    }
    
    if(tt==tt_all & tt_all %in% colnames(analysis_data_stan$x_tmax)){
      analysis_data_stan$x_tmax[,tt_all]=1
    } 
    if(tt!=tt_all & tt_all %in% colnames(analysis_data_stan$x_tmax)) {
      analysis_data_stan$x_tmax[,tt_all]=0
    }
    
    if(tt==tt_all & tt_all %in% colnames(analysis_data_stan$trt_mat)){
      analysis_data_stan$trt_mat[,tt_all]=1
    } 
    if(tt!=tt_all & tt_all %in% colnames(analysis_data_stan$trt_mat)) {
      analysis_data_stan$trt_mat[,tt_all]=0
    }
  }
  
  analysis_data_stan$x_intercept = 
    analysis_data_stan$x_intercept[analysis_data_stan$ind_start,,drop=F]
  analysis_data_stan$x_slope = 
    analysis_data_stan$x_slope[analysis_data_stan$ind_start,,drop=F]
  analysis_data_stan$x_tmax = 
    analysis_data_stan$x_tmax[analysis_data_stan$ind_start,,drop=F]
  analysis_data_stan$trt_mat = 
    analysis_data_stan$trt_mat[analysis_data_stan$ind_start,,drop=F]
